  ...
  class TestRunner
    ...
    def run_lua_specs
      ...
      @spec_files.each do |spec_file|
        ...
        # Use multipart format: zbspec.lua first, then helpers (from config), then spec_helper, then spec file (each with its own chunkname)
        # Format: ---FILE:filename---\ncontent\n---FILE:filename2---\ncontent2...
        zbspec_code = load_zbspec_lua
        lua_code = ''
        lua_code += "---FILE:zbspec.lua---\n#{zbspec_code}" unless zbspec_code.empty?
        load_helpers_list.each do |name, content|
          lua_code += "---FILE:lua/#{name}.lua---\n#{content}" unless content.empty?
        end
    ...
    # Load helper Lua files from ZBSpec/lua/ (names from config['helpers']), before spec_helper
    def load_helpers_list
      helpers = Array(config['helpers']).compact
      helpers.each_with_object([]) do |name, out|
        base = name.to_s.end_with?('.lua') ? name.to_s.chomp('.lua') : name.to_s
        next if base.empty?
        path = File.join(ZBSpec.root, 'lua', "#{base}.lua")
        next unless File.file?(path)
        puts "📦 Preloading lua/#{base}.lua" if verbosity >= 1
        out << [base, File.read(path) + "\n"]
      end
    end
